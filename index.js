...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const time = moment.tz('Brasil/Bras铆lia').format('DD/MM HH:mm:ss')
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`鉂� Falhou, no momento da convers茫o ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
				...
				case 'gtts':
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto 茅 bosta, tio')
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* NANDO'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nosso Canal: https://youtube.com/channel/UCUAWFYfMSlx-78EhJknehjg'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`Ol谩 o prefixo do seu bot agora 茅: ${prefix}`)
					break
				case 'lol':
				...
				case 'limpar':
					...
					break
				case 'teste':
					if (!isOwner) return reply('Voc锚 nao 茅 dono do bot haha')
					if (args.length < 1) return reply('Ola meu dono 鈾�')
					anu = await client.chats.all()
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse cara aqui @${mentioned[0].split('@')[0]} acabou de virar adm, entao respeitem 馃槑馃憤`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse cara aqui @${mentioned[0].split('@')[0]} Foi rebaixado para membro comum 馃槀!`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A que voc锚 deseja adicionar um g锚nio??')
					if (args[0].startsWith('08')) return reply('Use o c贸digo do pa铆s, mas')
					try {
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui levou ban com sucesso 馃檴 @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `Essa 茅 a lista de admins do grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('O modo Simi est谩 ativado cuidado macacada 馃檳')
						samih.push(from)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Modo simi ativado com sucesso neste grupo hehe 馃檴锔�')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desativando o modo simi com sucesso neste grupo 馃檮锔�')
					} else {
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Voce ja atuvou isso KAKKAKA lerd茫o')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Ativou com sucesso o recurso de boas-vindas neste grupo 馃槈')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Desativando com sucesso o recurso de boas-vindas neste grupo 馃檮')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
				case 'clone':
					...
					try {
						...
						mentions(`Ok, Agora minha foto de perfil est谩 igual a do @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Deu erro, aff')
					}
					break
